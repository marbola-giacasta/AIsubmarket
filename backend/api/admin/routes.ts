...
// ── GET /requests ─────────────────────────────────────────────
// For approved requests: joins the live tag so the frontend
// can show DNS state, renewal state and cancel date inline.
router.get('/requests', requireAuth, requireAdmin, async (_req, res, next) => {
  try {
    ...
    // One extra query to get ALL tags for approved fqdns
    ...
    if (approvedFqdns.length > 0) {
      const { data: liveTags } = await supabase
        .from('tags')
        .select('fqdn, dns_type, dns_value, subscription_cancelled, subscription_cancel_date, dns_updated_at')
        .in('fqdn', approvedFqdns);
      (liveTags || []).forEach(t => tagMap.set(t.fqdn, t));
    }

    // Enrich each approved request with its current tag state
    const enriched = (requests || []).map(r => {
      if (r.status !== 'approved') return r;
      const tag = tagMap.get(r.fqdn);
      return {
        ...r,
        // tag_exists: false means user cancelled and tag was already cleaned up before this fix
        tag_exists:           !!tag,
...
// ── GET /subdomains ───────────────────────────────────────────
// Two separate queries — no FK join needed, avoids FK definition issues
router.get('/subdomains', requireAuth, requireAdmin, async (_req, res, next) => {
  try {
    ...
    const subdomains = (tagsRes.data || []).map(t => ({
      ...t,
...
router.post('/requests/:id/propose-price', requireAuth, requireAdmin, async (req, res, next) => {
  try {
    ...
    await supabase.from('subdomain_requests').update({
      price_usd: price_usd?Number(price_usd):null, price_chf: price_chf?Number(price_chf):null,
      price_eur: price_eur?Number(price_eur):null, price_status:'proposed',
    }).eq('id', req.params.id);
...
router.post('/requests/:id/approve', requireAuth, requireAdmin, async (req, res, next) => {
  try {
    ...
    await supabase.from('tags').insert({ subdomain:r.subdomain, domain:r.domain, fqdn:r.fqdn, owner_id:r.requester_id, price_usd:pa?r.price_usd:null, price_chf:pa?r.price_chf:null, price_eur:pa?r.price_eur:null, subscription_active:true, subscription_cancelled:false });
