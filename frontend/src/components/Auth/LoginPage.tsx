// @ts-nocheck
// Mobile fix: email.trim().toLowerCase() before sending because
// iOS autocapitalize silently turns "mario@gmail.com" into "Mario@gmail.com"
// causing valid credentials to fail. Also added autoCapitalize/autoCorrect off
// and proper autoComplete hints for password managers.
...
export default function LoginPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const { token, user } = await fn(email.trim().toLowerCase(), pass);
  ...
  return (
    <div style={{ display:'flex', height:'100dvh', flexDirection: isMobile ? 'column' : 'row', overflow:'hidden' }}>
          ...
          <div style={s.termBody}>
            ...
            {['open-ai.ch','open-ai.live','geminai.info','course-ai.co.uk'].map(d => (
              <div key={d} style={{ paddingLeft:'52px', fontSize:'13px', marginBottom:'3px', fontFamily:'var(--font-mono)' }}>
                <span style={{ color:'var(--gold)' }}>"{d}"</span><span style={{ color:'var(--muted)' }}>,</span>
              </div>
            ))}
      ...
      <div style={{ ...s.formSide, flex: isMobile ? 1 : undefined, width: isMobile ? '100%' : 'clamp(300px,40vw,440px)', overflowY:'auto', borderLeft: isMobile ? 'none' : '2px solid var(--green)', borderTop: isMobile ? '2px solid var(--green)' : 'none' }}>
        <div className="fade-up" style={{ width:'100%', maxWidth:'380px' }}>
          {!isMobile && <div style={{ marginBottom:'24px' }}><span style={{ fontFamily:'var(--font-display)', fontSize:'20px', display:'block', marginBottom:'4px', letterSpacing:'1px' }}># SubMarket</span><span style={{ fontFamily:'var(--font-mono)', color:'var(--comment)', fontSize:'12px' }}>// AI subdomain registry</span></div>}
          ...
          <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
            <Field label="email"    type="email"    value={email} onChange={e => setEmail(e.target.value)} placeholder='"you@example.com"'
              autoCapitalize="none" autoCorrect="off" autoComplete="email" spellCheck={false} inputMode="email" />
            <Field label="password" type="password" value={pass}  onChange={e => setPass(e.target.value)}  placeholder={mode==='register' ? '"min 8 chars"' : '"........"'}
              autoCapitalize="none" autoCorrect="off" autoComplete={mode==='login' ? 'current-password' : 'new-password'} />
          ...
          <p style={{ marginTop:'16px', fontFamily:'var(--font-mono)', fontSize:'12px', color:'var(--muted)' }}>
            // <button style={{ background:'none', border:'none', color:'var(--teal)', fontFamily:'var(--font-mono)', fontSize:'12px', cursor:'pointer', textDecoration:'underline' }} onClick={() => setMode(mode==='login' ? 'register' : 'login')}>{mode==='login' ? 'switch to register.js' : 'switch to login.js'}</button>
          </p>
        </div>
      </div>
    </div>
...
function Field({ label, type, value, onChange, placeholder, ...rest }) {
  return (
    <div style={{ display:'flex', alignItems:'center', background:'var(--bg)', border:'1px solid var(--border)' }}>
      <span style={{ padding:'10px', fontFamily:'var(--font-mono)', fontSize:'12px', color:'var(--gold)', borderRight:'1px solid var(--border)', minWidth:'130px', whiteSpace:'nowrap', flexShrink:0 }}><span style={{ color:'var(--blue)' }}>const </span>{label}</span>
      <span style={{ padding:'0 8px', fontFamily:'var(--font-mono)', color:'var(--muted)', fontSize:'12px', flexShrink:0 }}>=</span>
      <input type={type} value={value} onChange={onChange} placeholder={placeholder} required {...rest}
...
function Line({ p, c, children }) {
  return <div style={{ display:'flex', gap:'10px', marginBottom:'3px', fontSize:'13px', fontFamily:'var(--font-mono)' }}><span style={{ color:'var(--comment)', minWidth:'40px', flexShrink:0 }}>{p}</span><span style={{ color:c }}>{children}</span></div>;
}
