// @ts-nocheck
// ─────────────────────────────────────────────────────────────
// LoginPage.tsx — mobile login fixes:
//
// Root cause of mobile failures:
//   iOS Safari autocapitalises the first letter of any text
//   input by default. So when the user types "mario@gmail.com"
//   the browser silently sends "Mario@gmail.com" — which does
//   not match the stored email and bcrypt returns 401.
//
// Fixes applied:
//   1. email.trim().toLowerCase() before every API call
//   2. autoCapitalize="none"   — disables iOS capitalisation
//   3. autoCorrect="off"       — disables iOS autocorrect
//   4. autoComplete="email"    — lets password managers work
//   5. inputMode="email"       — shows email keyboard on Android
//   6. spellCheck={false}      — no red underlines on email
//   7. password.trim()         — removes accidental spaces from
//                                autofill on some Android keyboards
// ─────────────────────────────────────────────────────────────
...
export default function LoginPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      // Normalise both fields before sending.
      // iOS silently capitalises emails and Android keyboards
      // sometimes add trailing spaces — these fixes handle both.
      ...
      const { token, user } = await fn(cleanEmail, cleanPass);
  ...
  return (
    <div style={{ display:'flex', height:'100dvh', flexDirection: isMobile ? 'column' : 'row', overflow:'hidden' }}>
      ...
      {/* ── Dark section ─────────────────────────────────── */}
      {isMobile ? (
        // Mobile: compact dark banner at top
        <div style={s.mobileBanner}>
          <div style={s.bannerTop}>
            <span style={s.bannerLogo}>
              # <span style={{ color:'var(--green)' }}>Sub</span>Market
            </span>
        ...
        <div style={s.terminal}>
          ...
          <div style={s.termBody}>
            ...
            {['open-ai.ch','open-ai.live','geminai.info','course-ai.co.uk'].map(d => (
              <div key={d} style={{ paddingLeft:'52px', fontSize:'13px', marginBottom:'3px', fontFamily:'var(--font-mono)' }}>
        ...
        <div className="fade-up" style={{ width:'100%', maxWidth:'380px' }}>
          ...
          {!isMobile && (
            <div style={{ marginBottom:'24px' }}>
              <span style={{ fontFamily:'var(--font-display)', fontSize:'20px', display:'block', marginBottom:'4px', letterSpacing:'1px' }}>
                # SubMarket
              </span>
              <span style={{ fontFamily:'var(--font-mono)', color:'var(--comment)', fontSize:'12px' }}>
                // AI subdomain registry
              </span>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
            ...
            {/* Email field — all mobile fixes here */}
            <div style={s.field}>
              <span style={s.fieldLabel}>
                <span style={{ color:'var(--blue)' }}>const </span>email
              </span>
            ...
            <div style={s.field}>
              <span style={s.fieldLabel}>
                <span style={{ color:'var(--blue)' }}>const </span>password
              </span>
              <span style={s.fieldEq}>=</span>
              <input
                type="password"
                ...
                placeholder={mode === 'register' ? '"min 8 chars"' : '"........"'}
                required
                autoCapitalize="none"
                autoCorrect="off"
            ...
            {error && (
              <div style={s.error}>ERR: {error}</div>
            )}

            <Btn type="submit" variant="primary" disabled={loading} style={{ justifyContent:'center', width:'100%' }}>
              {loading
                ? '// loading...'
                : mode === 'login'
                  ? '> authenticate()'
          ...
          <p style={{ marginTop:'16px', fontFamily:'var(--font-mono)', fontSize:'12px', color:'var(--muted)' }}>
            // {' '}
            <button
              style={{ background:'none', border:'none', color:'var(--teal)', fontFamily:'var(--font-mono)', fontSize:'12px', cursor:'pointer', textDecoration:'underline', padding:0 }}
...
const s = {
  ...
  modeTab:       { flex:1, padding:'8px 10px', background:'transparent', border:'none', borderBottom:'2px solid transparent', fontFamily:'var(--font-mono)', color:'var(--muted)', fontSize:'12px', cursor:'pointer', textAlign:'left', marginBottom:'-2px', transition:'color 0.1s, border-color 0.1s' },
  ...
  fieldLabel:    { padding:'10px', fontFamily:'var(--font-mono)', fontSize:'12px', color:'var(--gold)', borderRight:'1px solid var(--border)', minWidth:'130px', whiteSpace:'nowrap', flexShrink:0 },
