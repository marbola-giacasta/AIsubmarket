// @ts-nocheck
// ─────────────────────────────────────────────────────────────
// AdminRequests.tsx — shows all pending and resolved subdomain
// requests. I can approve or reject each one from here.
// Approving registers the subdomain in the tags table automatically.
// ─────────────────────────────────────────────────────────────
...
export default function AdminRequests() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { requests } = await req('GET', '/admin/requests');
  ...
  async function handleApprove(id) {
    setError(''); setMsg('');
    try {
      const { message } = await req('POST', `/admin/requests/${id}/approve`);
      setMsg(message);
      load();
    } catch (err) { setError(err.message); }
  }
  ...
  return (
    <div className="fade-up">
      ...
      <div style={s.sectionHead}>
        <span style={s.sectionLabel}>// PENDING [{pending.length}]</span>
      ...
      {resolved.length > 0 && (
        <>
          <div style={{ ...s.sectionHead, marginTop: '36px' }}>
            <span style={s.sectionLabel}>// RESOLVED [{resolved.length}]</span>
            <div style={s.sectionLine} />
          </div>
          <div style={s.cardGrid}>
            {resolved.map(r => (
...
const s = {
  ...
  sectionHead: { display: 'flex', alignItems: 'center', gap: '14px', marginBottom: '14px' },
  sectionLabel: { fontFamily: 'var(--font-display)', fontSize: '11px', color: 'var(--muted)', letterSpacing: '1px', whiteSpace: 'nowrap' },
