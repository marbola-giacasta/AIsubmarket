// @ts-nocheck
// ─────────────────────────────────────────────────────────────
// AdminDomains.tsx — ALL registered user subdomains with health.
// Shows: LIVE (DNS configured), NO DNS, DNS REMOVED, CANCELLED.
// Fetches tags + users via two separate queries (no FK needed).
// ─────────────────────────────────────────────────────────────
...
function getHealth(tag) {
  if (tag.subscription_cancelled) {
    return {
      label: 'CANCELLED', color: 'var(--muted)', icon: '∅',
  ...
  if (tag.dns_type && tag.dns_value) {
    return {
      label: 'LIVE', color: 'var(--comment)', icon: '✓',
  ...
  if (!tag.dns_type && tag.dns_updated_at) {
    return {
      label: 'DNS REMOVED', color: 'var(--orange)', icon: '⚠',
      note:  `DNS removed ${new Date(tag.dns_updated_at).toLocaleDateString('en-GB')} — subscription still active`,
    };
  }
  return {
    label: 'NO DNS', color: 'var(--red)', icon: '✗',
...
export default function AdminDomains() {
  ...
  const filtered = subdomains.filter(tag => {
    ...
    const ms = tag.fqdn.toLowerCase().includes(search.toLowerCase())
  ...
  const counts = subdomains.reduce((a, t) => {
    const l = getHealth(t).label;
    a[l] = (a[l] || 0) + 1;
    return a;
  }, {});
  ...
  return (
    <div className="fade-up">
      ...
      <div style={s.statsRow}>
        {[
          ...
          <button key={f} onClick={() => setFilter(f)} style={{
            ...s.statBtn,
            borderColor: filter === f ? c : 'var(--border)',
            background:  filter === f ? 'var(--surface-dark)' : 'var(--surface)',
          }}>
      ...
      {filtered.length === 0 && (
        <div style={s.empty}>
          {subdomains.length === 0
            ? '// no subdomains registered yet — approve a request first'
      ...
      <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fill,minmax(300px,1fr))', gap:'8px' }}>
        {filtered.map(tag => {
          const h     = getHealth(tag);
          const price = fmtP(tag.price_usd, tag.price_chf, tag.price_eur);
          return (
            <div key={tag.id} style={{ ...s.card, borderLeftColor: h.color }}>
              <div style={s.cardHead}>
                ...
                <span style={{ fontFamily:'var(--font-display)', fontSize:'9px', padding:'2px 7px', background:h.color, color:'#F8F8F8', letterSpacing:'1px', flexShrink:0 }}>
                  {h.label}
                </span>
              </div>
              <div style={s.cardBody}>
                ...
                {tag.dns_type && tag.dns_value && (
                  <div style={s.dnsBlock}>
                    <span style={{ ...s.dnsType, color:DNS_COLOR[tag.dns_type]||'var(--muted)', borderColor:DNS_COLOR[tag.dns_type]||'var(--muted)' }}>
                      {tag.dns_type}
                    </span>
                    <span style={{ fontFamily:'var(--font-mono)', color:'var(--muted)', fontSize:'11px' }}>→</span>
                    <span style={{ fontFamily:'var(--font-mono)', color:'var(--orange)', fontSize:'11px', wordBreak:'break-all' }}>
                      {tag.dns_value}
                    </span>
                    {!!tag.dns_proxied && (
                      <span style={{ fontFamily:'var(--font-display)', fontSize:'9px', color:'var(--teal)', border:'1px solid var(--teal)', padding:'1px 5px' }}>
                        CF PROXY
                      </span>
                    )}
                  </div>
...
const s = {
  ...
  statBtn:   { display:'flex', flexDirection:'column', alignItems:'center', padding:'8px 16px', border:'1px solid', cursor:'pointer', gap:'2px', transition:'all 0.1s', background:'var(--surface)' },
